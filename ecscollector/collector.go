...
var (
	...
	memWorkingSetDesc = prometheus.NewDesc(
		"ecs_container_memory_working_set_bytes",
		"Estimated container memory working set in bytes: runtime usage minus inactive file-list memory, clamped to zero.",
		containerLabels, nil)
...
func (c *collector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, container := range metadata.Containers {
		...
		if workingSet, ok := memoryWorkingSet(s.MemoryStats.Usage, memoryStats, inactiveFileKey); ok {
			ch <- prometheus.MustNewConstMetric(memWorkingSetDesc, prometheus.GaugeValue, float64(workingSet), containerLabelVals...)
		}
...
func memoryWorkingSet(usage uint64, stats map[string]uint64, inactiveFileKey string) (uint64, bool) {
	inactiveFile, ok := stats[inactiveFileKey]
	if !ok {
		return 0, false
	}
