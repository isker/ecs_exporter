...
* The working-set metric subtracts inactive file-list memory as a reclaimability
heuristic. It is not application heap size or an exact measure of memory
required to avoid an OOM.
* Memory breakdown metrics overlap and do not sum to a meaningful number.
* The container limit metric is sourced from ECS configuration; a task-level
fallback is shared capacity and must not be summed across containers as if it
were independently reserved for each one.
